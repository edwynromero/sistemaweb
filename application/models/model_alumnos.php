...
//            } else {
    ...
    blicfunctionguardarRazonesSociales($arrRazones, $conexion = null, $cod_alumno = null) {
        ...
        foreach ($arrRazones as $rowRazones) {
            ...
            $alumno = new Valumnos($conexion, $cod_alumno);
            $telD
            efaultAlumno = $alumno->getTelefonos(true);
    ...
    function guardarAlunoGeneralViejo($arrAlumnoCompleto)
    {
     ...
     * @return repuesta Guardar
     */
    function guardarAlunoGeneral($arrAlumnoCompleto)
    {
     ...
     */
    function cambiarEstado($codigo_alumno, $cod_usuario)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $conexion->trans_begin();
        $alumnos   = new Valumnos($conexion, $codigo_alumno);
     ...
     */
    function getAlumnosHabilitados($buscar, $separador)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('alumnos');
     ...
     */
    function listarAlumnosDatatableCtaCte($arrFiltros, $separador, $separadorDecimal)
    {
        $conexion = $this->load->database($this->codigofilial, true);
     ...
     * @return array $ctacte
     */
    function getCtaCte($cod_alumno, $condiciones = null, $debe = false, $orden = null)
    {
        $conexion = $this->load->database($this->codigofilial, true);
    ...
    function getCtaCteSinFacturar($codalumno, $condiciones, $orden, $separador, $soloCobradas = false)
    {
        $conexion = $this->load->database($this->codigofilial, true);
    ...
    function getCtaCteFacturaCobro($codalumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('cuentacorriente');
        $arrCondicion = array(
    ...
    function getAlumnos($buscar, $separador)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('alumnos');
        $arrCondindiciones = array();
    ...
    function getRazonSocialAlumno($cod_alumno, $paisFilial)
    {
        $conexion = $this->load->database($this->codigofilial, true);
...
//ver si se elimina
    function getCtaCteImputar($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('cuentacorriente');
        $this->load->helper('alumnos');
    ...
    function getReporteAlumnos($idFilial, $codigoDesde = null)
    {
    ...
    function getListadoCentroReportes($idFilial, $arrLimit = null, $arrSort = null, $search = null, $searchFields = null, $fechaDesde = null, $fechaHasta = null)
    {
        $conexion              = $this->load->database($idFilial, true);
    ...
    function getExamenAlumno($cod_alumno)
    {
        $conexion       = $this->load->database($this->codigofilial, true);
    ...
    function getResumenCuenta($idFilial, $codAlumno)
    {
        $conexion = $this->load->database($idFilial, true);
        $this->load->helper('cuentacorriente');
        ...
        foreach ($arrMatriculas as $matricula) {
            ...
            if ($matricula["estado"] == "habilitada") {
                $arrResp["estado"] = "activo";
            }
    ...
    function getConceptosCtaCteDebe($codalumno, $refinanciar = true, $arrmatriculas = null)
    {
        $conexion            = $this->load->database($this->codigofilial, true);
        $alumno              = new Valumnos($conexion, $codalumno);
        $conceptos           = array();
    ...
    function getMatriculasPeriodosAlumno($codalumno, $codplan, $agruparMatricula = false)
    {
        $conexion           = $this->load->database($this->codigofilial, true);
        $alumno             = new Valumnos($conexion, $codalumno);
    ...
    function getDetalleMateriasPlan($cod_alumno, $cod_plan_academico, $cod_periodo = null, $detalle = false)
    {
...
//                                if ($ocomision->getCodigo() == $comisionesdestino[$h]['codigo']) {
     ...
     * @return array $ctacte
     */
    function getCtaCteFrm($cod_alumno, $filial = null, $idioma = null)
    {
    ...
    function getNombreAlumno($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('alumnos');
        $objAlumno      = new Valumnos($conexion, $cod_alumno);
    ...
    function getMatriculasPeriodosInhabilitar($cod_alumno)
    {
        $conexion   = $this->load->database($this->codigofilial, true);
        $objAlumno  = new Valumnos($conexion, $cod_alumno);
        $estado     = 'habilitada';
    ...
    function ver_facturas_alumno($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('filial');
        $this->load->helper('alumnos');
    ...
    function getMatriculasPeriodos($cod_alumno, $estado = null, $sinestado = null)
    {
        $conexion   = $this->load->database($this->codigofilial, true);
        $objAlumno  = new Valumnos($conexion, $cod_alumno);
    ...
    function getCtaCteCambioVenc($cod_alumno, $cod_concepto, $concepto)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('cuentacorriente');
        $this->load->helper('filial');
    ...
    function getAlumnosMayorQue($cod_alumno)
    {
        $this->load->helper('alumnos');
        $conexion          = $this->load->database($this->codigofilial, true);
        $arrCondindiciones = array(
     ...
     * @return array razones sociales
     */
    function getRazonesSocialesAsignar($codigo_alumno)
    {
        $conexion   = $this->load->database($this->codigofilial, true);
        $arrRazones = array();
        if ($codigo_alumno == '-1') {
    ...
    function login_alumno($userName, $password)
    {
    ...
    function alertaLoginCampus($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $conexion->trans_start();
    ...
    function enviarLoginCampus($alerta, $objalerta, CI_DB_mysqli_driver $conexion = null, $cod_filial = null, &$comentario = null)
    {
        if ($conexion == null) {
            $conexion = $this->load->database($this->codigofilial, true);
        }
    ...
    function getProximasMesasExamenesDashboarCampus($cod_filial, $cod_alumno)
    {
    ...
    function getUltimasNotasCargadasDashboardCampus($cod_filial, $cod_alumno)
    {
        $conexion             = $this->load->database($cod_filial, true);
        $myAlumno             = new Valumnos($conexion, $cod_alumno);
    ...
    function getProximasClasesDashboardCampus($cod_filial, $cod_alumno)
    {
        $conexion       = $this->load->database($cod_filial, true);
        $myAlumno       = new Valumnos($conexion, $cod_alumno);
    ...
    function getProximosVencimientosDashboardCampus($cod_filial, $cod_alumno, $habilitado = null, $idioma = null)
    {
        $conexion = $this->load->database($cod_filial, true);
        $this->load->helper('cuentacorriente');
        $arrCondiciones = array(
...
// metodos campus viejo
    function validarDatosAlumnoLoginCampus($arrDatos)
    {
        $conexion          = $this->load->database('alumnos', true);
    ...
    function validarPrimerDatosAlumnoLoginCampus(&$conexion, $tipo_id, $identificacion, $fecha_nac, $cod_filial, $email, $code = null)
    {
    ...
    function registrarAlumnoCampus($arrDatos)
    {
        $conexion     = $this->load->database('alumnos', true);
    ...
    function validarSegundoDatosAlumnoLoginCampus(&$conexion, $objalumno, $lista, $asistencias = null, $cursos = null, $direcciones = null)
    {
        $validacion = false;
        $conexion->order_by('matriculas_periodos.codigo', 'desc');
        $matriculasper    = $objalumno->getMatriculasPeriodos(Vmatriculas_periodos::getEstadoHabilitada());
    ...
    function enviarMailConfirmarLogin($conexion, $email, $cod_alumno, $cod_filial, $md5)
    {
    ...
    function confirmarLoginCampus($code)
    {
        $conexion = $this->load->database('alumnos', true);
        $conexion->trans_start();
        $retorno['codigo'] = 0;
    ...
    function enviarMailLoginConfirmado($conexion, $email, $cod_alumno, $cod_filial)
    {
        $this->load->library('email');
    ...
    function restablecePassCampus($user)
    {
        $conexion = $this->load->database('alumnos', true);
        $conexion->trans_start();
        $retorno           = array();
    ...
    function enviarMailRestablecePassCampus($conexion, $md5, $cod_alumno, $cod_filial, $email)
    {
        $this->load->library('email');
    ...
    function modificoPassCampus($code, $pass)
    {
        $conexion          = $this->load->database('alumnos', true);
        $retorno['codigo'] = 0;
        $conexion->trans_start();
    ...
    function enviarMailConfirmaCambioPass($conexion, $md5, $cod_alumno, $cod_filial, $email)
    {
        $conexion = $this->load->database($cod_filial, true);
        $this->load->library('email');
    ...
    function getFilialLoginEnvioCampus($code)
    {
        $conexion = $this->load->database('alumnos', true);
        $retorno  = '';
        if ($code != '') {
    ...
    function unificarEmailCampus($cod_login, $mail)
    {
        $conexion = $this->load->database('alumnos', true);
        $conexion->trans_start();
        $cambios           = array();
    ...
    function restablecePassAnteriorCampus($code)
    {
        $conexion = $this->load->database('alumnos', true);
        $conexion->trans_start();
        $retorno['codigo'] = 0;
    ...
    function modificoPassCampusDesdePerfil($userName, $pass, $cod_filial)
    {
        $conexion          = $this->load->database('alumnos', true);
        $retorno['codigo'] = 0;
        $conexion->trans_start();
...
// Get alumnos es usado en el campus nuevo
    function getAlumnoCampus($codigo, $filial, $separador, $apellidoPrimero)
    {
    ...
    function getDetalleExamenAlumnno($cod_filial, $cod_examen, $cod_alumno, $cod_materia, $cod_inscripcion)
    {
        $conexion    = $this->load->database($cod_filial, true);
        $condiciones = array(
            "codigo" => $cod_examen,
    ...
    function validarInscripcionExamen($cod_filial, $cod_examen, $cod_estado_academico)
    {
        $conexion    = $this->load->database($cod_filial, true);
        $condiciones = array(
    ...
    function getCtaCteCobro($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('cuentacorriente');
        $arrCondicion = array(
    ...
    function getMailsAlumnoCampus($codigo, $filial)
    {
        $conexion    = $this->load->database($filial, true);
        $retorno     = array();
    ...
    function asignarResponsables($arrResponsables, $conexion = null, $cod_alumno = null, $responsable_relacion = null)
    {
        $transInt = false;
        if ($conexion == null) {
            $transInt = true;
    ...
    function getPlanesAcademicos($codalumno, $soloDatosPrincipales = false)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $alumno   = new Valumnos($conexion, $codalumno);
    ...
    function getDetallesAlertasEmailCampus($cod_alumno)
    {
    ...
    function reenviar_email_campus($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $conexion->trans_start();
    ...
    function getFacturasNC($cod_alumno)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        $this->load->helper('filial');
        $alumno    = new Valumnos($conexion, $cod_alumno);
    ...
    function getMatriculas($cod_alumno)
    {
        $conexion   = $this->load->database($this->codigofilial, true);
        $alumno     = new Valumnos($conexion, $cod_alumno);
    ...
    function getDocumentacion($plan = null)
    {
        $conexion = $this->load->database($this->codigofilial, true);
        if (!$plan) {
    ...
    function getDocumentacion_alumno($cod_matricula)
    {
        $conexion    = $this->load->database($this->codigofilial, true);
    ...
    function deleteTodaDocumentacion($cod_matricula)
    {
        $conexion = $this->load->database($this->codigofilial, true);
    ...
    function setMaterialEntregado($cod_matricula, array $doc)
    {
        $conexion = $this->load->database($this->codigofilial, true);
    ...
    function saveDocumentacion($cod_matricula, array $doc)
    {
        $conexion = $this->load->database($this->codigofilial, true);
    ...
    function getCursosDisponibles($id_filial, $cod_alumno)
    {
    ...
    function getMateriasDisponibles($id_filial, $cod_alumno)
    {
        $conexion = $this->load->database($id_filial, true);
        $alumnos  = new Valumnos($conexion, $cod_alumno);
    ...
    function getMateriasDisponiblesDeCurso($id_filial, $id_curso, $cod_alumno)
    {
        $conexion = $this->load->database($id_filial, true);
        $alumnos  = new Valumnos($conexion, $cod_alumno);
    ...
    function getClasesDisponiblesDeMateria($id_filial, $id_materia, $cod_alumno)
    {
        $conexion = $this->load->database($id_filial, true);
        $alumnos  = new Valumnos($conexion, $cod_alumno);
    ...
    function getMaterialesDidacticosDeClaseParaAlumnoDeFilial($id_clase, $codigo_alumno, $codigo_filial)
    {
    ...
    function getVideosDeMateriaParaAlumnoDeFilial($id_materia, $codigo_alumno, $codigo_filial)
    {
        $conexion     = $this->load->database($codigo_filial, true);
        $alumnos      = new Valumnos($conexion, $codigo_alumno);
        $config_fecha = 3;
    ...
    function getProximosVideos($id_clase, $codigo_alumno, $codigo_filial)
    {
        $conexion = $this->load->database($codigo_filial, true);
        $alumnos  = new Valumnos($conexion, $codigo_alumno);
    ...
    function getVideosAnteriores($id_clase, $codigo_alumno, $codigo_filial)
    {
        $conexion = $this->load->database($codigo_filial, true);
        $alumnos  = new Valumnos($conexion, $codigo_alumno);
    ...
    function tieneAccesoPlataformaElearning($id_filial, $cod_alumno)
    {
        $conexion = $this->load->database($id_filial, true);
        $alumnos  = new Valumnos($conexion, $cod_alumno);
...
//    public function getDatosDeCurso($id_filial, $codigo_curso) {
    ...
    function getProximoVideoEnVivo($codigo_alumno, $codigo_filial)
    {
        $conexion = $this->load->database($codigo_filial, true);
        $alumnos  = new Valumnos($conexion, $codigo_alumno);
        //esto deberia salir de una configuración
    ...
    function get_materias_plan_academico($cod_filial, $cod_alumno, $idioma = null, $estado = null)
    {
        $conexion = $this->load->database($cod_filial, true);
        if ($idioma == null) {
    ...
    function check_exist_alumno_campus($email)
    {
    ...
    function actualizar_email_campus($emailantes, $emailnuevo, $codalumno = null, $codfilial = null)
    {
    ...
    function alta_campus_nuevo($nombre, $apellido, $email, $sexo, $idioma, $cod_filial, $cod_alumno)
    {
    ...
    function resetear_password_campus_nuevo($cod_alumno, $cod_filial)
    {
        ...
        if (!isset($arrResp[0]['id_usuario'])) {
            return class_general::_generarRespuestaModelo($conexion, $arrResp, "No existe el usuario en el campus");
        }
    ...
    function deshabilitar_usuario_campus($cod_alumno, $cod_filial)
    {
    ...
    function habilitar_usuario_campus($cod_alumno, $cod_filial)
    {
